...
func StatefulSetRollingUpgrade(ctx pipeline.Context) {
	...
	if previousReplicas != replicas {
		...
		log.Info("replicas changed, update infinispan", "replicas", replicas, "previous replicas", previousReplicas)
		updateNeeded = true
	...
	container := provision.GetContainer(provision.InfinispanContainer, spec)
	...
	if ispnContr.Memory != "" {
		...
		if memRequests.Cmp(previousMemRequests) != 0 || memLimits.Cmp(previousMemLimits) != 0 {
			...
			log.Info("memory changed, update infinispan", "memLim", memLimits, "cpuReq", memRequests, "previous cpuLim", previousMemLimits, "previous cpuReq", previousMemRequests)
	...
	if ispnContr.CPU != "" {
		...
		if cpuReq.Cmp(previousCPUReq) != 0 || cpuLim.Cmp(previousCPULim) != 0 {
			...
			log.Info("cpu changed, update infinispan", "cpuLim", cpuLim, "cpuReq", cpuReq, "previous cpuLim", previousCPULim, "previous cpuReq", previousCPUReq)
			statefulSet.Spec.Template.Annotations["updateDate"] = time.Now().String()
			updateNeeded = true
		}
	}
