...
func ConsoleUrl(ctx pipeline.Context) {
	i := ctx.Instance()
	if !i.IsExposed() {
	...
	case infinispanv1.ExposeTypeLoadBalancer, infinispanv1.ExposeTypeNodePort:
		...
		if len(externalService.Spec.Ports) > 0 && i.GetExposeType() == infinispanv1.ExposeTypeNodePort {
		...
		} else if i.GetExposeType() == infinispanv1.ExposeTypeLoadBalancer {
