...
func ClusterStatefulSet(ctx pipeline.Context) {
	i := ctx.Instance()
	// Load the existing StatefulSet so that we can compare it spec with the spec generated by this pipeline execution
	if _, err := ctx.Resources().StatefulSets().Load(i.GetStatefulSetName()); err != nil {
		if !errors.IsNotFound(err) {
			ctx.RetryProcessing(fmt.Errorf("failed to load StatefulSet: %w", err))
		}
	}
}
