...
func (ispn *Infinispan) ApplyEndpointEncryptionSettings(servingCertsMode string, reqLogger logr.Logger) {
	...
	if encryption != nil && encryption.ClientCert == "" {
...
func (ispn *Infinispan) IsClientCertEnabled() bool {
	return ispn.Spec.Security.EndpointEncryption != nil && ispn.Spec.Security.EndpointEncryption.ClientCert != ClientCertNone
}
...
func (ispn *Infinispan) IsGeneratedSecret() bool {
	return ispn.IsAuthenticationEnabled() && (ispn.Spec.Security.EndpointSecretName == "" || strings.HasSuffix(ispn.Spec.Security.EndpointSecretName, consts.GeneratedSecretSuffix))
}
