...
func TestAuthorizationWithCustomRoles(t *testing.T) {
	...
	verify := func(failClient tutils.HTTPClient) {
		tutils.NewCacheHelper("succeed-cache", failClient).CreateWithDefault()
		schema := testKube.GetSchemaForRest(ispn)
...
func createCacheBadCreds(cacheName string, client tutils.HTTPClient) {
	...
	tutils.NewCacheHelper(cacheName, client).CreateWithDefault()
}
